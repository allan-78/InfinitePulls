...
const uploadIncomingImages = async (files = []) => {
  ...
  for (const file of files) {
    try {
      const uploadResult = await uploadToCloudinary(file.path, 'infinitepulls/cards');
      uploaded.push({
...
exports.getProduct = async (req, res) => {
  try {
    ...
    if (!product) {
      return res.status(404).json({ success: false, message: 'Card listing not found' });
    }
...
exports.updateProduct = async (req, res) => {
  try {
    ...
    if (!isAdminUser(req.user)) {
      return res.status(403).json({ success: false, message: 'Only admins can update store packs' });
    }

    const existingImages = parseExistingImages(req.body.existingImages);
    const imagesToDelete = currentProduct.images.filter(
      (oldImg) => !existingImages.some((img) =>
        (img.public_id && img.public_id === oldImg.public_id)
        || (img._id && img._id === oldImg._id)
        || (img.url && img.url === oldImg.url))
    );

    const newUploadedImages = req.files?.length ? await uploadIncomingImages(req.files) : [];
    ...
    updatePayload.discountedPrice = req.body.discountedPrice ? Number(req.body.discountedPrice) : null;
    updatePayload.discountPercentage = req.body.discountPercentage ? Number(req.body.discountPercentage) : null;
    ...
    const discountChanged = Boolean(
      ...
      && (
        ...
        || Number(currentProduct.discountedPrice) !== Number(updatePayload.discountedPrice)
        || Number(currentProduct.discountPercentage || 0) !== Number(updatePayload.discountPercentage || 0)
        || String(currentProduct.discountStartDate || '') !== String(updatePayload.discountStartDate || '')
        || String(currentProduct.discountEndDate || '') !== String(updatePayload.discountEndDate || '')
      )
    );
...
exports.getDeletedProducts = async (req, res) => {
  try {
    ...
    const productsWithReviews = await Promise.all(
      products.map(async (product) => {
        const reviewCount = await Review.countDocuments({ product: product._id, isActive: true });
        const productObj = product.toObject();
        productObj.isOnSale = checkDiscountActive(product);
...
exports.softDeleteProduct = async (req, res) => {
  try {
    ...
    if (!isAdminUser(req.user)) {
      return res.status(403).json({ success: false, message: 'Only admins can archive store packs' });
    }

    product.isActive = false;
    await product.save();

    res.status(200).json({ success: true, message: 'Card listing archived successfully' });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
exports.deleteProduct = async (req, res) => {
  try {
    ...
    if (!isAdminUser(req.user)) {
      return res.status(403).json({ success: false, message: 'Only admins can permanently delete store packs' });
    }
    ...
    res.status(200).json({ success: true, message: 'Card listing permanently deleted' });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
exports.getProductsOnSale = async (req, res) => {
  try {
    ...
    const productsWithReviews = await hydrateProductsWithReviewData(products, true);
...
exports.getDiscountNotifications = async (req, res) => {
  try {
    ...
    const newDiscounts = await Product.find({
      ...
      .select('name price discountedPrice discountPercentage images category condition');
    ...
    const endingSoonDiscounts = await Product.find({
      ...
      .populate('seller', 'name')
      .select('name price discountedPrice discountPercentage images category condition discountEndDate');
...
exports.getMyListings = async (req, res) => {
  try {
    if (!isAdminUser(req.user)) {
      return res.status(403).json({ success: false, message: 'Only admins can access store listings' });
    }
